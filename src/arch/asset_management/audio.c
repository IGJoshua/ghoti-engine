...
int32 loadAudio(const char *name)
{
    ...
    if(!existingAudio)
    {
        ...
        {
            LOG("Unable to load %s.ogg. Too few buffers", name);
            return -1;
        }

        LOG("Loading audio file: %s\n", name);
        AudioFile audioResource;
        sprintf(audioResource.name.string, "%s", name);

        UUID path;

        sprintf(path.string, "resources/audio/%s.ogg", name);
...
void freeAudio(AudioFile * audio)
{
	if (audio)
	{
		...
		free(audio->output);
		hashMapDelete(audioFiles, &audio->name);
